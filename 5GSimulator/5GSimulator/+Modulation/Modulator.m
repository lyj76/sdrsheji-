        ...
        switch obj.EqualizerType
            case 'One-Tap'
                ...
                elseif strcmpi(obj.ChannelEstimationMethod,'PilotAided')
                     % LS channel estimation for each MIMO channel 信道估计
                     % 子载波72和OFDM符号数15
                    pilotMatrix = obj.ChannelEstimator.PilotMatrix;
                    ...
                    % figure(1)
                    % surf(10*log10(abs(obj.Channel)));
                    % xlabel('OFDM symbols')
                    % ylabel('Subcarrier Index')
                    % zlabel('Estimated Channel Frequency Response (dB)');
                    % 
                    % figure(2)
                    % surf(10*log10(abs(perfectChannel)));
                    % xlabel('OFDM symbols')
                    % ylabel('Subcarrier Index')
                    % zlabel('Real Channel Frequency Response (dB)');
