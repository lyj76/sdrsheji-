...
scStr.simulation.sweepValue                 = linspace(110,110,1);      % Define parameter values to sweep over. A good starting point for the pathloss is 150 to 110.
...
scStr.simulation.userVelocity               = 0;                        % UE velocity in m/s(EPA_3km/h ETU_60km/h SUI-6_10km/h)
...
scStr.channel.powerDelayProfile             = 'AWGN';            %PedestrianA AWGN Flat VehicularA ExtendedPedestrianA ExtendedVehicularA
scStr.channel.K                             = 0;
scStr.channel.delta                         = 1;

%% Channel Estimation and Equalization Parameters
scStr.simulation.channelEstimationMethod    = 'PilotAided';             % pilot aided LS channel estimation 导频辅助LS信道估计
scStr.simulation.noisePowerEstimation       = false;
scStr.simulation.pilotPattern               = 'Diamond';                % LTE Downlink like pilot allocations
                                                                        %Diamond
                                                                        %Rectangular
scStr.simulation.equalizerType              = 'One-Tap';                % One-Tap equalizer
scStr.simulation.receiverTypeMIMO           = 'ZF';                     % Zero Forcing equalization 迫零均衡器
...
scStr.modulation.cqiTable                   = 0;                        %选择case
...
%scStr.modulation.precodingMatrix{2}         = 1/sqrt(2) * eye(1); 
scStr.modulation.mcs                        = 1;                        %选择调制方式和码率
...
% numerology setup
scStr.modulation.numerOfSubcarriers         = 24;                       % this corresponds to a 1.4MHz transmission
...
scStr.modulation.samplingRate               = 15.36e6;              % sampling rate  'Automatic'
...
% scStr.schedule.fixedScheduleDL{1}           = ['UE1:24'];
 scStr.schedule.fixedScheduleDL{1}           = ['UE1:24']; 
% scStr.schedule.fixedScheduleDL{1}           = ['UE1:84,none:12,UE2:96,none:12,UE3:96'];             
% scStr.schedule.fixedScheduleDL{1}           = ['UE1:12,none:12,UE2:24,none:12,UE3:12'];
% scStr.schedule.fixedScheduleDL{1}           = ['UE1:192,none:12,UE2:192,none:12,UE3:192'];
% scStr.schedule.fixedScheduleDL{1}           = ['UE1:36,none:36'];
% scStr.schedule.fixedScheduleDL{2}           = ['none:12,UE2:24'];

%scStr.schedule.fixedScheduleDL{1}           = ['none:72,UE1:228'];
%scStr.schedule.fixedScheduleDL{1}           = ['none:144,UE1:156'];
